...
async function doJb() {
  ...
  try {
    ...
    logger.info("===END===");
    logger.info("🎉 التهكير حصل بنجاح! تم تفعيل الجيلبريك!");
    const btn = document.getElementById("jeilbrek");
    if (btn) {
      btn.textContent = "✅ التهكير حصل!";
      btn.style.background = "linear-gradient(135deg, #10b981, #059669)";
      btn.disabled = false;
    }
    const uaEl = document.getElementById("UA");
    if (uaEl) {
      uaEl.innerHTML = "<span style='color:#4ade80;font-size:1.15rem;font-weight:700;'>🎉 التهكير حصل بنجاح! تم تفعيل الجيلبريك!</span>";
    }
  } catch (e) {
    ...
    const btn = document.getElementById("jeilbrek");
    if (btn) {
      btn.textContent = "❌ فشل التهكير.. حاول مجدداً";
      btn.disabled = false;
    }
    //mem.free_all();
  }
